...
router.put(
  "/username",
  [v.ensureUserLoggedIn, v.ensureValidUsernameInBody],
  async (req, res) => {
    try {
      const username = req.body.username;
      if (req.session.username === username) {
        res
          .status(400)
          .json({ error: `Username is already (${username})` })
          .end();
        return;
      }
      ...
      if (user) {
        ...
        res
          .status(200)
          .json({ message: `Username changed to (${username})` })
          .end();
      }
    } catch (error) {
      res
        .status(400)
        .json({ error: `Could not change the username!` })
        .end();
    }
  }
);
...
router.put(
  "/password",
  [v.ensureUserLoggedIn, v.ensureValidPasswordInBody],
  async (req, res) => {
    try {
      let user = await Users.updatePassword(
